...
// Tamanho do buffer circular. Deve ser potênca de 2 para eficiência (masking).
// 2048 ou 4096 é ideal para voz para capturar o comprimento de onda fundamental.
...
typedef struct {
    ...
    float window_size;   // Tamanho da "janela" ou grão (geralmente PS_BUFFER_SIZE - 1)
} pitch_shifter_t;

/**
 * Inicializa a estrutura do pitch shifter.
 * @param ps Ponteiro para a estrutura
 * @param pitch_factor Fator de mudança (ex: 0.5 = oitava abaixo, 2.0 = oitava acima)
 */
void pitch_shifter_init(pitch_shifter_t *ps, float pitch_factor);

/**
 * Processa uma única amostra (float) e retorna a amostra com pitch alterado.
 * @param ps Ponteiro para a estrutura
